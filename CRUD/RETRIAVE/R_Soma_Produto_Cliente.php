...
$sql2 = "SELECT `ProdutoID` as `Nome`, sum(`Volume`) as `Volume`,  format(sum(`Peso`), 3, 'pt-br') as `Peso` 
        FROM `estoque` WHERE ((`ProdutoID` = $TEMP_Produto ) AND (`ClienteID` = $TEMP_Cliente )
        AND (`Data_Estoque` between ('$Data_Inicio') AND ('$Data_Final') ) );";
