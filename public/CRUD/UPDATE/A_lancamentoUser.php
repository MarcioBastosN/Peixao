...
//        $strNumero = trim( str_replace( "R$", null, $strNumero ) );
    ...
    $idProduto = filter_input(INPUT_POST, "Peixe_A");
    if (is_numeric($idProduto)) {
        ...
        $p->setNome(filter_input(INPUT_POST, "Peixe_A"));
    ...
    $dadosLancamento->setId(filter_input(INPUT_POST, "cod"));
    $dadosLancamento->setCliente(filter_input(INPUT_POST, "Cliente_A"));
    ...
    $dadosLancamento->setNr(filter_input(INPUT_POST, "NR_A"));
    $dadosLancamento->setDiaAtual($data);
    $dadosLancamento->setDataLancamento(filter_input(INPUT_POST, "Data_A"));
    $dadosLancamento->setSituacao(filter_input(INPUT_POST, "Situacao_A"));

//    $peso = removerFormatacaoNumero(filter_input(INPUT_POST, "Peso_A"));
    $peso = limparPonto(filter_input(INPUT_POST, "Peso_A"));
    $peso = limparVirgula($peso);

    $volume = filter_input(INPUT_POST, "Volume_A");
    ...
    $dadosLancamento->setClass(filter_input(INPUT_POST, "Situacao_A"));
    $dadosLancamento->setObservacoes(filter_input(INPUT_POST, "Observacao_A"));

    if (empty($dadosLancamento->getId())) {
        $valida = false;
    ...
    if (empty($dadosLancamento->getPeso())) {
        $valida = false;
        array_push($dados, ['Peso' => "Peso, "]);
    }
    if (empty($dadosLancamento->getVolume())) {
        $valida = false;
        array_push($dados, ['Volume' => "Volume, "]);
    }
    ...
    if ($valida) {
        ...
        echo $lancamento->AtualizarLancamento($dadosLancamento);
    } else {
